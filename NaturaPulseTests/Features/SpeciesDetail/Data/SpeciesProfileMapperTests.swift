...
final class SpeciesProfileMapperTests: XCTestCase {
    private func dto(_ items: [(String?, String?)]) -> GBIFDescriptionsResponseDTO {
        GBIFDescriptionsResponseDTO(results: items.map {
            GBIFDescriptionDTO(type: nil, language: "eng", description: $0.0, source: $0.1)
        })
    }

    func testPicksFirstNonEmptyDescription() {
        let profile = SpeciesProfileMapper.map(dto([("   ", "s0"), ("", "s1"), ("A spotted dove.", "IOC")]))
